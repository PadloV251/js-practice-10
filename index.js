...
function customAssign(...objects) {
  ...
  for (const object of objects) {
    Object.assign(assignObj, object);
  }

  for (const key in assignObj) {
    if (typeof assignObj[key] === "number") {
      assignObj[key] = assignObj[key].toString();
    }
  }
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const freezed = Object.freeze(extended);
  if (Object.isFrozen(freezed)) {
    return freezed;
  } else {
...
function getObjectValuesSum(obj) {
  ...
  const reduceSum = valuesSum.reduce((accumulator, currentValue) => {
